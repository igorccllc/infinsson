...
// Decomposição: quanto veio de aporte (bolso, TOTAL — inclui imóvel) vs juros (rendimento
// de mercado, só sobre o pl). Imóvel não gera "juros" aqui de propósito — sua valorização
// não é modelada por este card, então aporte+juros deixou de somar pat exatamente (a
// diferença é a valorização do imóvel, não capturada em nenhum dos dois baldes).
function aporteRendimentoSeries() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const apo = h.apo ?? 0;        // aporte TOTAL — mostrado como está
    const apoPL = apoPLOf(h);      // só o que foi pra pl — base do juros (mercado)
    let rMes;
    if (i === 0) { aAcum = apo; rAcum = (h.pl || 0) - apoPL; rMes = rAcum; }
    else { rMes = ((h.pl || 0) - (HISTORICAL[i - 1].pl || 0)) - apoPL; aAcum += apo; rAcum += rMes; }
  ...
  // % um do outro (aporte vs juros), não % do patrimônio total — pat agora inclui a
  // valorização do imóvel, que não está em nenhum dos dois baldes.
  const explained = curA + curR;
  return {
    labels, aporte, rend, pat, crossMonth,
    curPat, curA, curR,
    aPct: explained ? curA / explained * 100 : 0,
    rPct: explained ? curR / explained * 100 : 0,
    last12A, last12R,
  };
}
