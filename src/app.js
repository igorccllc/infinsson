...
// Saldo devedor total das dívidas ativas HOJE (avança o cronograma de cada uma pela debtNow).
// É o passivo que o balanço precisa deduzir para o Patrimônio Líquido ser honesto.
function totalDebtNow() {
  return (S.debts || []).reduce((s, d) => {
    const n = debtNow(d);
    return s + (n.quitada ? 0 : n.saldoAtual);
  }, 0);
}
...
// Retorno mensal realizado da carteira: (Δinvestível - aporte) / investível anterior.
// Usa 'pl' (patrimônio investível, exclui imóvel), não 'pat' — o imóvel é ilíquido e não
// marcado a mercado, então sua variação polui o retorno da carteira financeira. Cai pro 'pat'
// só se a linha não tiver 'pl'. O aporte ainda carrega ruído, mas converge na média de 12m+.
function realizedReturns() {
  ...
  for (let i = 1; i < HISTORICAL.length; i++) {
    ...
    if (!base) continue;
    out.push({ d: cur.d, r: (curV - base - (cur.apo || 0)) / base });
  }
  return out;
}
...
const METRIC_DOCS = {
  runway: () => {
    ...
    return {
      title: 'Runway',
      what: 'Quantos meses a carteira banca seus gastos se toda renda parar hoje.',
      calc: [
        ['Carteira investida (aba Patrimônio)', fmt(liquid)],
        ['÷ Gasto médio 12m (Histórico, col. Gastos)', fmt(avgGas) + '/mês'],
        ['= Runway', Math.round(liquid / avgGas) + ' meses (' + (liquid / avgGas / 12).toFixed(1) + ' anos)'],
      ],
      ignores: 'Rendimento da carteira e inflação dos gastos — trata o dinheiro como caixa parado.',
