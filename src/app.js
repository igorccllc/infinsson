...
// Patrimônio Total (inclui imóveis e ativos não investíveis) — usado para FI
...
function renderExpenses() {
  ...
  const secEntries = Object.entries(bySecMap).sort((a, b) => b[1] - a[1]);
  ...
  el.innerHTML = `
    ...
    <div class="kpi-grid mb-16">
      ...
      <div class="kpi">
        ...
        <div class="kpi-sub"><span class="${deltaPct <= 0 ? 'badge-up' : 'badge-down'}">${deltaPct <= 0 ? '↓' : '↑'} ${deltaPct > 0 ? '+' : ''}${fmtPct(deltaPct)}</span> vs ${monthLabel(prevMonth)}</div>
      </div>
      <div class="kpi">
        <div class="kpi-label">Maior Seção</div>
