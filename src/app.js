...
function _buildFluxoGridTab() {
  ...
  // corpo = linhas depois do cabeçalho com rótulo na 1ª coluna
  const body = FLUXOGRID.slice(hIdx + 1).filter(r => String(r[0] ?? '').trim() !== '');
  ...
  return `<div class="card">
    ...
    <div class="table-wrap fluxo-grid-wrap" style="max-height:600px;overflow:auto">
      <table>
        ...
        <tbody>${body.map(r => `<tr>${visibleCols.map(j => {
          const v = r[j];
          const isNum = typeof v === 'number';
          return `<td class="${j > 0 ? 'r ' : ''}${colKind[j] === 'resumo' ? 'accent' : ''}${isNum && v < 0 ? ' red' : ''}">${fmtCell(v)}</td>`;
        }).join('')}</tr>`).join('')}</tbody>
      </table>
    </div>
  </div>`;
