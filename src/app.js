...
const METRIC_DOCS = {
  ...
  finumber: () => {
    ...
    return {
      ...
      assumes: isPerp
        ? 'Você vive só do juro real: o principal se preserva para sempre em valor de hoje.'
        : 'Saque de fração fixa ao ano; em sequências ruins de retorno o principal pode se esgotar em ~30 anos.',
    };
  },
  fidate: () => {
...
function buildGastosTabHtml() {
  ...
  const sortedDesc = matched.slice().sort((a, b) => b.d.localeCompare(a.d));
...
function renderDebts() {
  ...
  if (!debts.length) {
    el.innerHTML = `
      <div class="page-header"><div>
        ...
        <div class="page-subtitle">Registre financiamentos e acompanhe saldo, juros e quitação</div>
      </div>
  ...
  el.innerHTML = `
    <div class="page-header"><div>
      <div class="page-title">Dívidas</div>
      <div class="page-subtitle">${ativos.length} ativa(s) · saldo evolui pelo cronograma desde o registro</div>
    </div>
    <div class="page-actions"><button class="btn btn-primary btn-sm" onclick="openDebtModal()">+ Nova dívida</button></div>
    </div>
...
function _debtsDrawChart() {
  ...
  for (let m = 0; m <= maxMonths; m++) {
    labels.push(m % 12 === 0 || m === maxMonths ? monthLabel(addMonths(curKey, m)) : '');
    totals.push(+infos.reduce((s, x) => {
      const i = x.now.idx + m;
  ...
  activeCharts.debts = makeLineChart('ch-debts', {
    labels,
    datasets: [{ label: 'Saldo devedor total', data: totals, borderColor: '#f87171', backgroundColor: '#f8717115', tension: .3, pointRadius: 0, borderWidth: 2, fill: true }],
  });
}
...
function openDebtModal(id) {
  ...
  openModal(d ? 'Editar Dívida' : 'Nova Dívida', `
    ...
    <div class="form-hint" style="margin-bottom:14px">O saldo informado é o da data escolhida — o app evolui o cronograma sozinho a partir dela.
      Se a parcela real difere da teórica (seguros MIP/DFI, taxa de administração, TR), informe-a: o app usa o valor real e mostra a diferença como encargos.</div>
    <div class="form-actions">
      <button class="btn btn-secondary" onclick="closeModal()">Cancelar</button>
