...
function _buildFluxoGridTab() {
  ...
  const rowHtml = ({ cells, section }) => {
    const label = String(cells[0] ?? '');
    const secColor = section ? (SECAO_COLORS[label] || 'var(--accent)') : '';
    const firstStyle = section && secColor ? ` style="border-left:3px solid ${secColor}"` : '';
    return `<tr class="${section ? 'fx-section' : 'fx-detail'}">${visibleCols.map((j, idx) => {
      ...
      return `<td class="${cls}"${idx === 0 ? firstStyle : ''}>${fmtCell(v)}</td>`;
    }).join('')}</tr>`;
  };
  ...
  return `<div class="card">
    <div class="flex-between mb-8">
      ...
      <span style="font-size:11px;color:var(--text-dim)">espelho da planilha · macro-contas em destaque</span>
    </div>
    ${toggle}
...
function renderDebts() {
  ...
  el.innerHTML = headerHtml(`${ativos.length} ativa(s) · saldo evolui pelo cronograma desde o registro`) + tabsHtml + `
    ...
    ${ativos.length ? (() => {
      ...
      const saldoHoje = ativos.reduce((s, x) => s + x.now.saldoAtual, 0);
      return `<div class="card">
        ...
        <table>
          ...
          <tbody>${yearly.map(r => `
            <tr>
              ...
              <td class="r muted">${fmtPct((1 - r.saldo / saldoHoje) * 100)}</td>
            </tr>`).join('')}
          </tbody>
        </table>
      </div>
