...
async function syncFromSheets(silent = false) {
  ...
  try {
    ...
    if (Array.isArray(data.fluxo) && data.fluxo.length > 0) {
      localStorage.setItem('finplan_fluxo_secoes', JSON.stringify(data.fluxo));
    ...
    if (Array.isArray(data.fluxoBold) && data.fluxoBold.length > 0) {
      ...
      localStorage.setItem('finplan_fluxobold', JSON.stringify(FLUXOBOLD));
      changed = true;
    }
  ...
  return `
    <div class="param-layout">
      <div class="param-panel">
        ...
        <div class="form-hint" style="margin-top:8px">O crescimento <b>real</b> de cada receita/gasto (acima da inflação) é definido item a item no <a class="src-link" onclick="navigate('cashflow');return false" href="#">Fluxo de Caixa</a> — <b>0 = sobe só com a inflação</b>. Cenários de crescimento agregado ficam na aba Cenários.</div>
      </div>
      <div class="param-panel">
      ...
      <div class="table-wrap">
        <table class="history-table">
          <thead><tr>
            ...
            <th class="r">Aporte</th>
            <th class="r">Rentabilidade</th>
            <th class="r">Taxa de Poupança</th>
            <th class="r">Receita</th>
            <th class="r">Gastos Totais</th>
            <th class="r">Gastos Recorrentes</th>
            <th class="r">Gastos Não-Rec.</th>
            <th class="r">Taxes</th>
          </tr></thead>
          <tbody>${[...HISTORICAL].reverse().map(h=>{
            const apo  = h.apo ?? (h.rec - h.gas);
            const rate = h.txp ?? savingsRate(h);                 // G: prefere a coluna da planilha
  ...
  const kpiTotaisHtml = ativos.length > 1 ? `
    <div class="kpi-grid mb-16">
      ...
      <div class="kpi"><div class="kpi-label">Juros até quitar tudo</div><div class="kpi-value" style="color:var(--yellow)">${fmt(jurosRestTot)}</div>
        <div class="kpi-sub">custo futuro total da dívida</div></div>
    </div>` : '';
...
function renderProtection() {
  ...
  const age = currentAge();
  const anosApos = Math.max(0, (S.assumptions.retirementAge || 60) - age);
  ...
  el.innerHTML = `
    ...
    <div class="card mt-16">
      ...
      <div class="grid-2" style="gap:12px">
        ...
        <div class="form-group"><label class="form-label">Dívida coberta por MIP (R$)</label>
          <input class="form-input" type="number" min="0" step="1000" value="${dividaCoberta}" placeholder="padrão: toda dívida ativa" onchange="updateProtection('dividaCobertaMIP', this.value)"></div>
...
// ── 19d. OBJETIVOS (metas paralelas à FI) ─────────────────
function renderObjetivos() {
  ...
  const headerHtml = (sub) => `
    ...
    <div class="page-actions"><button class="btn btn-primary btn-sm" onclick="openGoalModal()">+ Nova meta</button></div>
    </div>`;

  if (!goals.length) {
    el.innerHTML = headerHtml('Metas paralelas à independência — e quanto cada uma empurra a data da FI') + `
      <div class="card" style="padding:40px;text-align:center;color:var(--text-muted)">
        <p style="font-size:16px;margin-bottom:8px">Nenhuma meta cadastrada.</p>
        <p style="font-size:13px">Cadastre objetivos com data (educação dos filhos, imóvel, sabático, troca de carro). Cada um consome patrimônio na data e o app mostra o impacto na sua FI.</p>
      </div>`;
    return;
  }
...
function saveGoal(id) {
  ...
  saveState(); closeModal(); renderObjetivos();
}

function deleteGoal(id) {
  if (!confirm('Excluir esta meta?')) return;
  S.goals = S.goals.filter(x => x.id !== id);
  saveState(); renderObjetivos();
}
