...
function _buildMilestonesCard() {
  ...
  const body = rows.map(r => `
    <tr>
      <td class="bold">${fmtK(r.t)}</td>
      <td class="muted">${monthLabel(r.d)}</td>
      <td class="r ${r.faster ? 'green' : r.slower ? 'yellow' : ''}">${fmtDur(r.delta)}${r.faster ? ' ↓' : r.slower ? ' ↑' : ''}</td>
      <td class="r muted">${fmtDur(r.fromStart)}</td>
    </tr>`).join('');
  ...
  return `
    <div class="card mt-16">
      <div class="flex-between mb-8">
        ...
        <span style="font-size:11px;color:var(--text-dim)">de R$ 100 mil em 100 mil · patrimônio total</span>
      </div>
      <div class="table-wrap">
        <table>
          <thead><tr>
            <th>Marco</th><th>Atingido em</th><th class="r">Desde o anterior</th><th class="r">Total desde o início</th>
          </tr></thead>
          <tbody>${body}</tbody>
        </table>
      </div>
