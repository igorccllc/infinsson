...
function buildMetodoTabHtml() {
  ...
  const allMonths = [...new Set(MOBILLS.map(r => r.d))].sort().reverse();
  const filterMonth = allMonths.includes(expMetFilter) ? expMetFilter : 'all';
  const rows = filterMonth === 'all' ? MOBILLS : MOBILLS.filter(r => r.d === filterMonth);

  const monthOptions = `<option value="all" ${filterMonth === 'all' ? 'selected' : ''}>Todos os meses (${MOBILLS.length} lançamentos)</option>` +
...
function renderExpenses() {
  ...
  requestAnimationFrame(() => {
    ...
    // Pizza — distribuição por método de pagamento (aba Método)
    if (metodoChartData && document.getElementById('ch-exp-met-pie')) {
      destroyChart('expMetPie');
      activeCharts.expMetPie = makePieChart('ch-exp-met-pie',
        metodoChartData.labels, metodoChartData.data, metodoChartData.colors);
    }
